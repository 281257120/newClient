<template>
    <div class='search' :style="{position:oPosition.position,width:preview ? '318px' : '',left:authority == 'view' ? '0' : ''}">
        <header :style="{'background-color':oPosition.backgroundColor}">
            ...
            <section class="input" :style="{'color':sFontColor}">
                <router-link :to="{name:'search'}" v-if="authority == 'view'"></router-link>
                <i class="iconfont icon-search"></i>{{searchValue}}
            </section>
            <router-link :to="{name:'message'}" class="message" v-if="bMessage && authority == 'view'" :style="{'color':sFontColor}">
                <!-- <span></span> -->
